...
public abstract class PluginSearchPacketHandler<T extends CustomPayload> implements IPluginClientPlayHandler<T>
{
    ...
    {
        @Override
        public void receive(PluginSearchPacket payload, ClientPlayNetworking.Context context)
        {
            this.receivePlayPayload(payload, context);
        }
    };

    public static PluginSearchPacketHandler<PluginSearchPacket> getInstance()
    {
        return INSTANCE;
    }
    ...
    public void encodePayload(List<String> action, List<String> dimension, List<String> block, List<String> entityType, List<String> item, List<String> tag, int range, String source, String timeBefore, String timeAfter, int pages, RolledbackMode rolledBack)
    {
        ...
        INSTANCE.sendPlayPayload(new PluginSearchPacket(ledgerSearch.getSearchData(), pages));
    }
